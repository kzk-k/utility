...
riot.tag2('app-contents', '<todo></todo> <div class="card"> <div class="card-content"> <inputurl check-input-url="{urlInputCompleted}"></inputUrl> </div> </div> <div class="card"> <div class="card-content"> <datepicker></datePicker> </div> </div> <div class="center-align"> <btn-send></btn-send> </div>', 'app-contents,[data-is="app-contents"]{ display: block; flex: 1; padding: 3.2rem; }', '', function(opts) {
		this.show_flg = false;
...
riot.tag2('datepicker', '<div class="input-field"> <input id="datepickerId-1" type="text" class="datepicker datepicker1" oninput="{checkInputDatepicker}"> <label for="datepickerId-1">期間指定</label> {test.value} </div> <div class="input-field"> <input id="datepickerId-2" type="text" class="datepicker datepicker2"> <label for="datepickerId-2">期間指定</label> </div>', 'datepicker .date-text::after,[data-is="datepicker"] .date-text::after{ content: \'日\'; }', '', function(opts) {
	...
	this.checkInputDatepicker = function() {
...
riot.tag2('inputurl', '<div class="input-field"> <i class="material-icons prefix">textsms</i> <input type="text" id="inputUrl" oninput="{opts.checkInputUrl}"> <label for="inputUrl">URLを入力</label> </div> </div>', 'inputurl,[data-is="inputurl"]{ display: block; flex: 1; } inputurl .material-icons,[data-is="inputurl"] .material-icons{ margin-top: 0.5rem; } inputurl label,[data-is="inputurl"] label{ height: 2rem; }', '', function(opts) {
});
